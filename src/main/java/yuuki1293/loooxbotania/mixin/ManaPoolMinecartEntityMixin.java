...
public abstract class ManaPoolMinecartEntityMixin {
    ...
    @Redirect(method = "moveAlongTrack", at = @At(value = "FIELD", target = "Lvazkii/botania/common/block/block_entity/mana/ManaPumpBlockEntity;comparator:I", opcode = Opcodes.PUTFIELD))
    private void moveAlongTrack(ManaPumpBlockEntity pump, int value) {
        pump.comparator = (int) ((double) getMana() / (double) NEW_MAX_MANA * 15);
    }

    @ModifyArg(method = "getComparatorLevel", at = @At(value = "INVOKE", target = "Lvazkii/botania/common/block/block_entity/mana/ManaPoolBlockEntity;calculateComparatorLevel(II)I"), index = 1)
